...
Elf ElfParser::parse(std::ifstream &elf_stream, std::string elf_name)
{
    ...
    {
        ElfProgramHeader program_header = (header.word == ElfHeader::WordSize::b32) ? parse_program_header32(elf_stream) : parse_program_header64(elf_stream);
        program_headers.emplace_back(program_header);
    }

    //todo: read section headers
...
ElfProgramHeader ElfParser::parse_program_header32(std::ifstream &elf_stream)
{
